...
author: "Glenn De Lange" # Remember to add your name!
date: "21-10-2024"
theme: united
---
...
cantril %>%
  ...
  mutate(`Absolute change` = `2023` - `2011`,
         `Relative change` = scales::percent((`2023` - `2011`) / `2011`, accuracy = 0.01)) %>%
  reactable(
    columns = list(
      `2011` = colDef(format = colFormat(digits = 2)),
      `2023` = colDef(format = colFormat(digits = 2)),
      `Absolute change` = colDef(format = colFormat(digits = 2))
    )
  )
```
